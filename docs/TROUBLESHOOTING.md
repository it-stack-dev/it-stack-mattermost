# Troubleshooting — IT-Stack MATTERMOST
...
docker compose logs --tail=50 mattermost
...
1. Check logs: docker compose logs mattermost
2. Verify environment variables are set correctly
3. Check database connectivity: pg_isready -h lab-db1 -p 5432
4. Verify port is not already in use: ss -tlnp | grep 8065
...
```bash
# Test connectivity
psql -h lab-db1 -U mattermost_user -d mattermost_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-mattermost
...
| Log | Path |
|-----|------|
| Application | docker compose logs mattermost |
